...
- `endpoint.rs`: parses `ELASTICSEARCH_URL`, lifting `user:password@` out of the URL into request auth and trimming a trailing slash, so both services build requests the same way.
