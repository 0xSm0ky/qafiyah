...
describe('cn', () => {
  ...
  it('keeps a type-scale size alongside a text color', () => {
    expect(cn('text-display', 'text-text')).toBe('text-display text-text');
    expect(cn('text-heading', 'text-text-muted')).toBe('text-heading text-text-muted');
    expect(cn('text-caption', 'text-text-subtle')).toBe('text-caption text-text-subtle');
    expect(cn('text-xxs', 'text-text')).toBe('text-xxs text-text');
  });

  it('still collapses conflicting sizes and conflicting colors', () => {
