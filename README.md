...
`bun run ci` ([`scripts/ci.ts`](scripts/ci.ts)) is the full gate: lint and format, type and repo checks, unit tests, contract snapshots (OpenAPI, generated client, Elasticsearch queries), and, with Docker, database-backed tests and smoke tests against the built stack. The pre-commit hook runs it without Docker and the pre-push hook runs the Docker phase. GitHub Actions runs only a fast subset (oxlint, format, types) plus gitleaks. Clippy denies `unwrap`, `expect`, `panic`, indexing, and lossy casts in production code. See [`docs/testing.md`](docs/testing.md).
